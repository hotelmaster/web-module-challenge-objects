...
function getLongReviews(reviews) {
  // need a new array to store the longest feedback strings
  let longStrings = [];
  // variable to count spaces in feedback string
  let numSpaces = 0;
  // variable for number of words in string
  let numWords = 0;
  // need to see which reviews[i].feedback has more than 15 words
  // first for-loop to check each index of reviews array
  for(let i in reviews) {
    // nested for-loop will search through the feedback string within the reviews[i].feedback[index] for index between 0 and the length of the feedback string
    for(let index = 0; index < reviews[i].feedback.length; index++) {
        if(reviews[i].feedback[index] == " ") {
          // increment spaces to use later to calc number of words
          numSpaces++;
      }
    }
    // number of words is always one more than number of spaces
      numWords = numSpaces + 1;
      // reset numSpaces to 0
      numSpaces = 0;
      // if longer than 15 words use push method to store in longStrings array
      // note that this stretch task wants the entire review pushed, not only the feedback string
      if(numWords > 15) {
        longStrings.push(reviews[i]);
      }
      // reset numwords to 0 for next feedback string
      numWords = 0;
      // end of for-loop checking index within feedback string
    // end of for-in loop checking index = i review
  }
  // exited both for-loops, now return longStrings array
  return longStrings;
}
